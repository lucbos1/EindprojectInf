...
if (isset($_POST["submit"])) {
    ...
    if (llnrExistsB($conn, $leerlingNr) !== false) {
        header("location: ../vragenlijst_B.php?error=already_submitted");
        exit(); 
    }
    ...
    for ($i = 1; $i <=80; $i++) {
        ...
        array_push($antwoorden_B, $antwoord);
    }

    $LG = 0; $CK = 0; $HA = 0; $PS = 0; $SA = 0; $RL = 0; $UV = 0; $ZP = 0; $SV = 0; $SW = 0;

    foreach ($antwoorden_B as $value) {
...
saveVragenlijstB($conn, $leerlingNr, $LG, $CK, $HA, $PS, $SA, $RL, $UV, $ZP, $SV, $SW);

}
else {
    header("location: ../vragenlijst_B.php?error=wrongsubmit");
    exit();  
}